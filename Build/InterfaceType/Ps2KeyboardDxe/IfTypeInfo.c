...
typedef struct _TYPE__gEfiPs2PolicyProtocolGuid{
  const CHAR8* TypeName;
  const UINTN TypeClass;
  const UINTN TypeSize;
  const GUID* ProtocolGuid;
  const CHAR8* InterfaceStructName;
} TYPE__gEfiPs2PolicyProtocolGuid;

TYPE__gEfiPs2PolicyProtocolGuid TypeInfo__gEfiPs2PolicyProtocolGuid = {
  .TypeName = "gEfiPs2PolicyProtocolGuid",
  .TypeClass = TYPE_CLASS_PROTOCOL,
  .TypeSize = sizeof(GUID),
  .ProtocolGuid = &gEfiPs2PolicyProtocolGuid,
  .InterfaceStructName = "EFI_PS2_POLICY_PROTOCOL"
};
...
void *TypeInfoList_LocateProtocol[] = {
  ...
  &TypeInfo__gEfiPs2PolicyProtocolGuid,
