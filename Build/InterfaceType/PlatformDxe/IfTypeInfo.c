...
TYPE__gEfiDevicePathProtocolGuid TypeInfo__gEfiDevicePathProtocolGuid = {
  ...
  .InterfaceStructName = "PKG_DEVICE_PATH"
};


typedef struct _TYPE__PKG_DEVICE_PATH{
  const CHAR8* TypeName;
  const UINTN TypeClass;
  const UINTN TypeSize;
...
} TYPE__PKG_DEVICE_PATH;

TYPE__PKG_DEVICE_PATH TypeInfo__PKG_DEVICE_PATH = {
  .TypeName = "PKG_DEVICE_PATH",
  .TypeClass = TYPE_CLASS_STRUCTURE,
  .TypeSize = sizeof(PKG_DEVICE_PATH),
  .StructFieldNum = 2,
