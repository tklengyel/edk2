...
typedef struct _TYPE__XENIO_PROTOCOL{
  const CHAR8* TypeName;
  const UINTN TypeClass;
  const UINTN TypeSize;
  const UINTN StructFieldNum;
  const CHAR8* StructFieldName_1;
  const CHAR8* StructFieldTypeName_1;
} TYPE__XENIO_PROTOCOL;

TYPE__XENIO_PROTOCOL TypeInfo__XENIO_PROTOCOL = {
  .TypeName = "XENIO_PROTOCOL",
  .TypeClass = TYPE_CLASS_STRUCTURE,
  .TypeSize = sizeof(XENIO_PROTOCOL),
  .StructFieldNum = 1,
  .StructFieldName_1 = "GrantTableAddress",
  .StructFieldTypeName_1 = "EFI_PHYSICAL_ADDRESS"
};
